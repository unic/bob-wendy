...
Describe "Set-ScUserConfigValue" {
  Context "Create user config if it does not exist" {
    ...
    It "Should have create Bob.config.user" {
      Test-Path  "TestDrive:\App_Config\Bob.config.user" | Should Be $true
    }
    It "Should have an configuration node in Bob.config.user" {
      ([xml](Get-Content "TestDrive:\App_Config\Bob.config.user")).SelectSingleNode("/Configuration") | Should Not  BeNullOrEmpty
    }
  }
  ...
  Context "Should add value if it doesn't exist yet" {
    ...
    It "Should have added dummyKey node with value dummy " {
      ([xml](Get-Content "TestDrive:\App_Config\Bob.config.user")).Configuration.dummyKey | Should Be  "dummy"
    }
  }
...
"@ | Out-File "TestDrive:\App_Config\Bob.config.user" -Encoding UTF8
      Set-ScUserConfigValue -Key dummyKey -Value New

      It "Should have updated dummyKey node with value dummy" {
        ([xml](Get-Content "TestDrive:\App_Config\Bob.config.user")).Configuration.ChildNodes[0].InnerText | Should Be  "New"
        ([xml](Get-Content "TestDrive:\App_Config\Bob.config.user")).Configuration.ChildNodes[0].Name | Should Be  "dummyKey"
      }

      It "Should not have touched superDummy" {
        ([xml](Get-Content "TestDrive:\App_Config\Bob.config.user")).Configuration.ChildNodes[1].Name | Should Be  "superDummyKey"
        ([xml](Get-Content "TestDrive:\App_Config\Bob.config.user")).Configuration.ChildNodes[1].InnerText | Should Be  "superDummy"
...
"@ | Out-File "TestDrive:\App_Config\Bob.config.user" -Encoding UTF8
    ...
    It "Should have kept comments" {
      $content = [xml](Get-Content "TestDrive:\App_Config\Bob.config.user")
